...
const GENERATED_BY = 'agent-workflow-init 0.1.0';
...
This workspace uses a tool-agnostic agent workflow generated by ${GENERATED_BY}.
...
function makeCursorRule() {
  return `---
description: "Use the shared agent workflow core and team profile for feature delivery, review, testing, and release preparation."
alwaysApply: true
---

Read AGENTS.md, workflow/team-profile.yaml, and workflow/core/ before acting on workflow tasks.

Implementation requires the feature branch gate, stage gate, and same-repo parallel gate.
Remote Git operations, branch creation, push, build/deploy triggers, database writes, and production config writes are manual-only.
`;
}
