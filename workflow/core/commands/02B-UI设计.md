...
- 显式触发才出图并留痕：仅当用户明确要求生成 UI 设计图/视觉稿/原型图时才产出设计文件。**可点击 HTML 原型统一走 `/02C-HTML原型` 阶段**——它强制先提取 design tokens 与组件清单，再产出受规范约束的单文件原型，避免自由发挥；生成后在 `02B-UI设计.md` 记录产物清单与文件路径，并标注其为草稿设计意图还是已确认设计。
- 不削弱 04A Gate：无论是否出图，`/04A-前端代码实现` 仍以 `02B-UI设计.md` 的设计基线（及 02C 原型，如有）为准。
