...
执行阶段按 `workflow/core/checklists/` 逐单核对并记录"命中 / 不适用 / 未核查原因"：

- `test-blind-spots.md` §A-10（环境差异）：识别"测试环境无法复现"的项并显式标注"上线后必验"。
- `data-consistency-review.md` §D：本期含 DML/补数脚本时，核对 COUNT 钉死、预演库真跑、恢复条件登记的执行证据。
- `branch-hygiene.md` §B：测试通过后、发布前，完成发布候选与生产基线的差异核查留痕。
- `validation-change-review.md` §C：校验口径变更时，对接口、导入、页面和异步链路分别实测，不得以一条链路代替其他链路。
- `third-party-integration-review.md` §B/§C/§E：第三方集成需实测业务码、幂等成功、可重试/永久失败、异步闭环以及发布后路由可达。
