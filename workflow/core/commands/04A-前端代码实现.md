...
- If `prototype/model.json`, `provenance.json`, or a 02D target report exists, verify source IDs/model hashes and the mapped/loss result for pages, visible text, and major components; target client `NOT_VERIFIED` is not implementation PASS evidence.
