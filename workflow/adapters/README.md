...
- Claude Code: `CLAUDE.md` + `.claude/commands/`（兼容格式）+ `.claude/skills/agent-workflow/SKILL.md`（官方推荐格式入口）。分阶段中文命令暂留 commands，待非 ASCII skill 命名支持明确后整体切换。
...
| Claude Code | `.claude/commands/`（兼容）；新推荐 `.claude/skills/` | commands + agent-workflow skill | ✅ 官方文档核实 |
