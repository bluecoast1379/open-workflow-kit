...
- `workflow/adapters/support-matrix.yaml` 明确披露 4 个 native 和 3 个 compatible 入口，未经真实工具验收的 native 没有被标记为 `native_verified`。
...
9. 使用 starter kit 外部的私有 denylist 运行 `bin/check-sanitized.cjs --extra-banned <private-denylist-file>`。
10. 在有私有规则源的环境运行 `bin/check-rule-catalog.cjs --provenance workflow/local/rule-provenance.private.yaml`。
11. 由维护者手动发布到目标渠道。
