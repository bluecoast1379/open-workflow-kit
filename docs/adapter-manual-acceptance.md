...
| 工具 | 操作 | 预期结果 |
| --- | --- | --- |
| Codex Desktop | 输入 `/04`，在 Skills 分组选择 `04-代码实现` | 选择后插入 Skill mention；不把它记录成字面 `/04-代码实现` 命令支持 |
| Codex CLI / IDE | 使用 `/skills` 或 `$workflow-04-code-implementation` | 可按 slug 或中文展示名找到 `04-代码实现` |
...
| Trae / Trae CN | 打开 Settings > Skills & Commands，并在输入框 `/` 面板搜索 `04` | `.trae/commands/04-代码实现.md` 的直接 Command 可用；同时启用 Codex 时可另见共享 Skill，但不得再有 `.trae/skills/<stage>` 产生的第三份入口 |
...
8. Trae 额外确认每阶段只有一份 Trae-owned Command；若同时启用 Codex，单独记录共享 `.agents/skills/` 的额外可见性。
...
- Kiro IDE 与 CLI、Copilot 不同 IDE、Trae 与 Trae CN 的客户端版本都应注明边界。
